        ...
        if (CanOpenApp) {
            if (RL.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $.post('http://bb-phone/GetMentionedTweets', JSON.stringify({}), function(MentionedTweets){
                        RL.Phone.Notifications.LoadMentionedTweets(MentionedTweets)
                    })
                    $.post('http://bb-phone/GetHashtags', JSON.stringify({}), function(Hashtags){
                        RL.Phone.Notifications.LoadHashtags(Hashtags)
                    })
                    if (RL.Phone.Data.IsOpen) {
                        $.post('http://bb-phone/GetTweets', JSON.stringify({}), function(Tweets){
                            RL.Phone.Notifications.LoadTweets(Tweets);
                        });
                    }
                } else if (PressedApplication == "bank") {
                    $.post('http://bb-phone/GetBankData', JSON.stringify({}), function(data){
                        RL.Phone.Functions.DoBankOpen(data);
                    });
                    $.post('http://bb-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        RL.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('http://bb-phone/GetInvoices', JSON.stringify({}), function(invoices){
                        RL.Phone.Functions.LoadBankInvoices(invoices);
                    });
                } else if (PressedApplication == "whatsapp") {
                    $.post('http://bb-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                        RL.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('http://bb-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        RL.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('http://bb-phone/GetSuggestedContacts', JSON.stringify({}), function(suggested){
                        RL.Phone.Functions.SetupSuggestedContacts(suggested);
                    });
                    $.post('http://bb-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('http://bb-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('http://bb-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        RL.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('http://bb-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "crypto") {
                    $.post('http://bb-phone/GetCryptoData', JSON.stringify({
                        crypto: "qbit",
                    }), function(CryptoData){
                        SetupCryptoData(CryptoData);
                    })

                    $.post('http://bb-phone/GetCryptoTransactions', JSON.stringify({}), function(data){
                        RefreshCryptoTransactions(data);
                    })
                } else if (PressedApplication == "racing") {
                    $.post('http://bb-phone/GetAvailableRaces', JSON.stringify({}), function(Races){
                        SetupRaces(Races);
                    });
                } else if (PressedApplication == "houses") {
                    $.post('http://bb-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                } else if (PressedApplication == "meos") {
                    SetupMeosHome();
                }  else if (PressedApplication == "lawyers") {
                    $.post('http://bb-phone/GetCurrentLawyers', JSON.stringify({}), function(data){
                        SetupLawyers(data);
                    });
                } 
            }
...
RL.Phone.Notifications.Add = function(icon, title, text, color, timeout) {
    $.post('http://bb-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
...
RL.Phone.Functions.LoadPhoneData = function(data) {
    ...
    $.post('http://bb-phone/GetLangData', JSON.stringify({}), function(langs){
